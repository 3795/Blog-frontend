...
export class WriteArticleComponent implements OnInit {
  ...
  initArticle(): void {
    ...
    if (this.id != undefined) {
      this.backendService.get("/article/" + this.id)
        .subscribe((data) => {
          if (data.code % 2) {
   ...
   */
  initOptions(): void {
    this.backendService.get("/category/cascade")
      .subscribe((data) => {
        if (data.code%2) {
  ...
  addArticle(): void {
    ...
    this.backendService.post("/article", body)
      .subscribe((data) => {
        if (data.code % 2) {
          this.msg.success(data.msg);
          this.router.navigateByUrl("/manage/console/article/list")
        } else {
          this.msg.error(data.msg);
        }
      });
  ...
  updateArticle(): void {
    ...
    this.backendService.put("/article", body)
      .subscribe((data) => {
        if (data.code % 2) {
          this.msg.success(data.msg);
          this.router.navigateByUrl("/manage/console/article/list")
